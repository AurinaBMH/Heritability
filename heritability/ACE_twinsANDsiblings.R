...
data = readMat(sprintf("~/GoogleDrive/Genetics_connectome/Heritability/data/output/twinEdges_%s_%s_%s%d.mat",parcellation, tract, weights, conDens))
...
heritmodels[1,4] <- twinACEFit@output$status$status
...
heritmodels[2,4] <- twinAEFit@output$status$status
...
heritmodels[3,4] <- twinCEFit@output$status$status
...
write.csv(heritabilityACE,sprintf("heritabilityACE_%s_%s_%s%d.txt",parcellation, tract, weights, conDens),row.names=FALSE)